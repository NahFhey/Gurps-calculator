...
- [!] Fix setTimeout race in src/components/ui/ConfirmDialog.tsx using the `cancelled` ref flag pattern. Add unit test simulating unmount-before-timeout. [auto-deferred 2026-05-03: file has no setTimeout — only synchronous useEffect with keydown listener and body overflow, no race exists]
