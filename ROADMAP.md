...
**Gap closure (all five audited gaps shipped 2026-09-01 — 15a COMPLETE):**
- ✅ **Inventory -> character-sheet equipment bridge SHIPPED 2026-09-01** (codex-shepherd, spec `2026-09-01-equipment-bridge-15a.md`, merge `codex/equipment-bridge`, verdict accepted — zero shepherd fixes; design [`docs/EQUIPMENT_BRIDGE_15A_PLAN.md`](./docs/EQUIPMENT_BRIDGE_15A_PLAN.md), 13 decisions): explicit promotion with move semantics — atomic `inventory/itemPromoted`/`itemDemoted` bus actions, `EquipmentCargo` blob + `Equipment.sourceItem` provenance for lossless round trips, `EquipItemModal` confirm form (campaign-currency cost label), Equip… entry points (PartyStashView picker + CharacterInventoryPanel), Send-to-pack with draft reconciliation, crafting stamps starter stats, partial stacks with re-stack-by-source-id, schema 1.6.1. 253 targeted tests; browser-verified promote→encumbrance shift→demote→re-stack→cargo-pre-filled re-promote. Follow-ups: sheet-side "Add from inventory" picker; tool-weight-on-sheet (own design pass).
- ✅ **Combat injury persistence SHIPPED 2026-09-01** (codex-shepherd, spec `2026-09-01-injury-persistence-15a.md`, merge `codex/injury-persistence`, verdict accepted — zero shepherd fixes; design [`docs/INJURY_PERSISTENCE_15A_PLAN.md`](./docs/INJURY_PERSISTENCE_15A_PLAN.md), 14 decisions): top-level `Character.status` (conditions/crippled/dead), catalog `persistsAfterCombat` flag, trimmed open-ended conditions with `revealed` carried, seed-back into combat + replace-on-sync (party-only), `dead||unconscious` downtime gate with rest-leader carve-out, Rest wake-up rule at HP>0, GM status editor + party-column badges + rest-strip listing, schema 1.6.2. 891 tests across 69 files; browser-verified full loop (persist → badges → gate → Add-All skip → re-seed into combat → rest wake → editor clear). Follow-ups: crippling permanence tiers, sheet status section (player-visibility question), condition recovery rules.
- ✅ **Multi-group `currentLocationId` scoping SHIPPED 2026-09-01** (direct fix, branch `fix/locid-scoping`): `currentLocationId` now re-derives from the active group's position whenever the active group changes — explicit `party/setActiveGroup` and the implicit reassignment when a drained group dissolves (`syncCurrentLocationToActiveGroup` in journeyEngine, hooked in the campaignReducer party branch). Mirrors arrival semantics: pinned location switches + `location.changed` log; wilderness follows tile terrain. Non-active arrivals still discover without switching. 5 new reducer tests. Full per-character location scoping (each group's downtime seeing its own fishing spots simultaneously) remains a larger follow-up — this fix makes the active-group model consistent, not per-group-parallel.
...
- ✅ Map editor polish (2026-09-01, user request): terrain brush with size 1–5 and circle/square shapes (`getBrushTiles`), Ctrl+scroll brush size + Shift+scroll paint elevation on the canvas (`onModifierWheel`), paint HUD overlay, elevation stepper buttons + wheel-blur guard on the number input (no more accidental scroll edits), `map/stampTerrain` extended with elevation + edge expansion (one action per brush stamp = one undo step). **Fixed inverted camera grab-pan** in `dragPan` and remapped mouse controls per Devin: right-drag pans, middle (scroll-click) drag rotates, left-drag is reserved for click/paint/token-drag — hand-tested by Devin 2026-09-01, feels good. Image layers gained **size-to-grid** (2026-09-01, user request): enter the imported map's printed grid cols/rows (rows auto-derived from image aspect until touched) and each image cell becomes one tile with the corner snapped; plus Snap-to-tiles and Fit-map buttons in `ImageLayersDialog`. Added Roll20-style **Align 3×3** (2026-09-02, user request): per-layer button enters an align mode on the map — drag a box over any 3×3 block of the image's printed grid (live 3×3 preview rect on the layer's elevation plane, Esc cancels) and the layer is rescaled so each image cell is one tile with the box corner snapped to the nearest intersection (`alignImageLayerToGrid` in `src/utils/imageAlign.ts`, drag capture in `MapScene`). Verified live in Chrome on the test map's beach battlemap.
- ✅ Map expansion + storage fixes (2026-09-02, user bug reports): **`expandMap` now offsets image layers** when rows/cols are prepended — layers are positioned in grid coordinates, so top/left expansion shifted the origin out from under them and the image visibly drifted off the terrain. **Storage moved from localStorage to IndexedDB** (`src/utils/storage.ts`, same async interface): edge expansion had grown the test map to 162×145 = 23,490 tiles ≈ 4.5M chars of JSON, bursting localStorage's ~5MB origin quota (saves failing with QuotaExceededError). IndexedDB has GB-scale quota; existing localStorage values migrate lazily on first read (copy removed after the IndexedDB write commits) and the wrapper falls back to localStorage where IndexedDB is missing, so jsdom tests are unaffected. Verified live: 4.89M-char campaign blob migrated, saves round-trip with revision stamping. Manual width/height edits in `ImageLayersDialog` now **resize around the image's center** (x/y shift by half the size change; grid ops — Align 3×3, Size to grid, Snap — stay corner-anchored, per Devin's request 2026-09-03). Longer-term: per-tile UUID objects (~200 chars each, id repeated 3×) are the real weight — a compact tile serialization is worth considering if maps keep growing.
- Responsive layout improvements — **DESIGNED (2026-09-01)**, grill-me session complete: 15 decisions in [`docs/UX_DESIGN_15C_PLAN.md`](./docs/UX_DESIGN_15C_PLAN.md). Desktop-only (900px→ultrawide, half-snap critical), shell shape kept, auto-collapse with manual pin, progressive disclosure for combat tracker, per-screen scope = combat tracker + character sheet + table overflow safety net. Implementation is phase 3 of the plan's sequence; prototype fold-order/multi-column first.
- Dark mode / theme support — **PHASE 1 SHIPPED (2026-09-01)** (codex-shepherd, spec `2026-09-01-theme-tokens-15c.md`, squash `eaadaed`, verdict accepted — zero shepherd fixes): semantic CSS-variable tokens across 244 files via committed idempotent codemod, grep-gate wired as `npm run check:tokens` + gurps-verify step 2b.5, dev-only light toggle, four no-op shade classes fixed, light-end strays hand-fixed. Verified: 4,209/4,209 tests, 58 token values byte-exact, browser dark pixel-identical + light legible. **PHASE 2 SHIPPED (2026-09-01)** (codex-shepherd, spec `2026-09-01-modal-primitive-15c.md` + continuation spec after a mid-run session kill, merge `4f7b007`, verdict accepted-with-fixes): shared `Modal` primitive (portal, `role="dialog"`/`aria-modal`/`aria-labelledby`, hand-rolled focus trap + restore, stacking-safe Escape + counter scroll-lock, token-native) replacing ~36 hand-rolled overlays across 31 files; `ConfirmDialog` refactored onto it, public API unchanged; UnifiedShell's two inline modals migrated (delete-character → ConfirmDialog). Justified leave-as-is: MapHeader click-catchers, ConditionAddPopover (popover), TravelWizard (side panel — wrong dialog role corrected to labeled region). Shepherd fix: modal-stack effect reads callbacks via refs so background re-renders can't steal topmost (+ regression test). Verified: 4,218/4,218 tests, tsc clean, token gate green, build ok, browser-exercised (portal/aria/focus/Escape/scroll-lock/backdrop-parity). This lands most of 15d's modal items. Remaining: phase 3 (responsive). Design — **DESIGNED (2026-09-01)**, same session/doc: tokens-first, dark-only shipped (dev-only light toggle as validation harness). CSS variables (RGB triplets) → Tailwind names; neutral chrome + 5 semantic statuses; mechanical class-swap (slate folds into gray, otherwise pixel-identical) + shared Modal primitive (lands most of 15d's dialog a11y); CI grep-gate; 3D map exempt. Sequence: tokens → Modal → responsive. Phase 1 is a codex-shepherd candidate.
...
- **Interior rendering questions**: token behavior under a slab (camera occlusion, selection), auto-hide roof when the active group is inside, fog/vision inside enclosed spaces. **Expanded (Devin, 2026-09-02 — load-bearing, this is what replaces exploded maps):** (1) *elevation cutaway keyed to selected token* — token at elevation N hides/ghosts everything ≥ N+1 (structure slabs + tall terrain columns) so interiors are navigable; plumbing half-exists (per-layer `visible` toggle → drive it from selection; or a three.js clipping plane at cut height for a true dollhouse cutaway — prototype: hard-hide vs ghost vs clip). (2) *Token POV mode* — camera at token position + eye height showing what it sees with blockers; v1 is visual truth only (geometry occludes naturally), rules-truth targeting waits on structure-layer v2 LOS.
...
- **Airship combat theaters — anchored frames** (Devin, 2026-09-02; replaces his Roll20 workaround of 3 maps exploded onto one canvas): recommended architecture from the initial discussion — NOT one big map (moving a ship + decks + tokens + shield every turn is the known pain) and NOT edge-extension nesting (decks aren't adjacent to the arena, they're *inside it at a moving, rotating pose*). Instead: deck maps **anchor to the ship entity**, ship has a pose (position/heading/altitude) on an arena-scale map; token world position = compose(ship pose, deck offset, deck level), scale conversion inherent in the transform. Consequences: ship movement = move one token (everything aboard rides along); cross-map range (rifle→lancer, deck gun→far ship) = distance between composed world positions in yards, GURPS 1/2D-Max applies directly; shield = barrier entity riding the ship's pose, gates boarding/attacks; targeting UI = per-combat "theater" (arena + anchored deck maps), target picker spans the theater with computed ranges, viewed surface is just a tab. LOS v1 simplification: world-space ray vs coarse occluders only (hull silhouettes, shield, arena obstacles) — below-decks not targetable from outside, per-tile geometry only for same-deck fights. The Roll20 exploded layout survives as an optional *view* convention over this model. **Sphere refinement (Devin, 2026-09-02):** every vehicle's local frame has a fixed spherical extent — the local map covers the whole bubble (decks + surrounding airspace), so escorts/lancers inside it ride the ship's motion as a moving reference frame; crossing the sphere is the deterministic frame handoff (exit → arena at composed position, enter → join ship's frame); the shield, when present, coincides with the sphere; shieldless vehicles keep it as pure geometry. Range math stays point-to-point on composed world positions (sphere-edge measurement would distort short-range/boarding bands; sphere's job is frames/membership/handoff/shield, not measurement). Open design question: frame ownership when spheres overlap during boarding (probable: explicit attachment — last deck touched / GM assignment). Depends on: tactical scale tier, vehicle-anchored links, structure-layer v2.
- **Person-scale combat battlemaps — 1 yd/square (GURPS B384)** (Devin, 2026-09-01): much of the plumbing exists — `Participant.position?: {q,r}`, `MovementRecord`, `CombatMapPanel` rendering participants + vision on a linked map's grid, and image-layer size-to-grid for imported battlemaps (15c). Missing: the 1-yard scale tier (`MapScale` bottoms out at 12 mi/tile — same gap as interiors, one tier lower), and the tactical-rules design questions: hex vs square presentation (data is axial q/r; 3D renderer draws square tiles), movement points per maneuver, reach/facing, and whether tactical maps are authored standalone or generated from a zoomed world tile.
