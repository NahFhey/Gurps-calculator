...
**Remaining:** the map surface only — table verification 2026-08-19 confirmed tracker + timeline surfaces and all player-view filtering. The map surface gained a `combat.mapId` writer later that day (6cea50a Battle Map selector) and its latent SyncProvider crash was fixed, but the condition entry point remains unreachable: the map combat layout is player-locked (CombatContext-local `gmMode` defaults false with no setter wired in that layout). Needs a GM-mode wiring decision — see COMBAT_CONDITION_VISIBILITY_PLAN.md.
