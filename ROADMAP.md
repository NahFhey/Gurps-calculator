...
- **Inventory -> character-sheet equipment bridge** (largest gap): `ItemInstance` carries no weight/category/stats, so crafted/looted gear tagged to a character can never reach `gcsData.equipment`, encumbrance, DR, or combat stats. **Designed 2026-09-01** (grill-me session, 13 decisions; see [`docs/EQUIPMENT_BRIDGE_15A_PLAN.md`](./docs/EQUIPMENT_BRIDGE_15A_PLAN.md)): explicit promotion with move semantics (one ledger at a time), cargo blob on `ItemInstance` for round-trip stat fidelity, confirm form at promote, crafting stamps starter stats, partial stacks with re-stack-by-source-id, atomic `itemPromoted`/`itemDemoted` bus actions, items-only, grandfathered. Ready for codex-shepherd dispatch.
