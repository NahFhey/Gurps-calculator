...
**Remaining:** the map surface only — table verification 2026-08-19 confirmed tracker + timeline surfaces and all player-view filtering, but found the map surface unreachable (`combat.mapId` has no writer; no UI links combat to a map). Needs a combat↔map link design decision.
