...
function MemberChip({
  ...
  return (
    <div
      ...
      className={`flex items-center gap-1.5 rounded border border-edge-strong bg-surface-1 p-1.5 ${isDragging ? 'opacity-50' : ''}`}
    >
      {image ? (
        <img src={image} alt={character.name} className="h-7 w-7 rounded-full object-cover" />
      ) : (
        <span className="flex h-7 w-7 items-center justify-center rounded-full bg-accent-700 text-[10px] font-bold text-white">
          {nameInitials(character.name)}
        </span>
      )}
      <span className="min-w-0 flex-1 truncate text-[11px] text-fg-primary">{character.name}</span>
      <button
        type="button"
        ...
        className="rounded px-1 text-xs text-fg-muted hover:bg-surface-2 hover:text-white"
      >
...
function PartyDropColumn({
  ...
  return (
    <section
      ...
      className={`min-h-28 rounded border p-1.5 ${isOver ? 'border-accent-400 bg-accent-950/30' : 'border-edge bg-surface-0/40'}`}
    >
      <h4 className="mb-1.5 text-[11px] font-semibold text-fg-primary">{label}</h4>
      <div className="space-y-2">
        {sources.map(({ group, members }) => {
          ...
          return (
            <div key={group.id}>
              <div className="mb-1 truncate text-[9px] uppercase tracking-wide text-fg-faint">{group.name}</div>
...
export function TravelStep1Party({
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-fg-muted">Choose who travels and how they go.</p>
      ...
      {travelingMemberIds.length === 0 && (
        <p role="alert" className="text-xs text-danger-300">Someone has to travel</p>
      )}

      <fieldset className="space-y-1.5">
        <legend className="text-[10px] font-semibold uppercase tracking-wide text-fg-muted">Conveyance</legend>
        <label className={`rounded bg-surface-0/40 p-2 text-xs ${footAllowed ? 'block cursor-pointer text-fg-primary' : 'block text-fg-faint'}`}>
        ...
        {vehicles.map(({ vehicle, type }) => {
          ...
          return (
            <label key={vehicle.id} className={`block rounded bg-surface-0/40 p-2 text-xs ${allowed ? 'cursor-pointer text-fg-primary' : 'text-fg-faint'}`}>
              <span className="flex items-center gap-2">
                <input
                  type="radio"
