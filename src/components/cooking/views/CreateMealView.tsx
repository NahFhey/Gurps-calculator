  ...
  return (
    <div className="bg-gray-800 rounded-lg p-6 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div><label className="block mb-2">People</label><input type="number" value={numPeople} onChange={(e) => onNumPeopleChange(e.target.value)} className="w-full bg-gray-700 px-3 py-2 rounded" /></div>
      </div>
      <div>
        <div className="flex justify-between mb-2">
          <label>Ingredients</label>
          <button onClick={onAddIngredient} className="bg-green-600 px-3 py-1 rounded text-sm"><Plus size={16} className="inline" /> Add</button>
        </div>
        {selected.map(i => (
          <div key={i.id} className="flex gap-2 mb-2">
            <select value={i.foodId} onChange={(e) => onIngredientFoodChange(i.id, e.target.value)} className="flex-1 bg-gray-700 px-3 py-1 rounded">
              {foods.map(f => <option key={f.id} value={f.id}>{f.name} ({f.types.join('/')}) - {f.quantity} lbs</option>)}
            </select>
            <input type="number" value={i.amount} onChange={(e) => onIngredientAmountChange(i.id, e.target.value)} className="w-20 bg-gray-700 px-3 py-1 rounded" />
            <button onClick={() => onRemoveIngredient(i.id)} className="text-red-400"><Trash2 size={18} /></button>
          </div>
        ))}
      </div>
      ...
      <div className="bg-gray-700 p-4 rounded space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <select value={selectedWorker} onChange={(e) => onWorkerChange(e.target.value)} className="w-full bg-gray-600 px-3 py-2 rounded">
          ...
          <div>
            ...
            <select value={selectedKitchenId} onChange={(e) => onKitchenChange(e.target.value)} className="w-full bg-gray-600 px-3 py-2 rounded">
          ...
          <div>
            ...
            <input type="number" value={cookingSkillValue} onChange={(e) => onCookingSkillChange(e.target.value)} className="w-full bg-gray-600 px-3 py-2 rounded" placeholder="e.g., 14" />
          </div>
      ...
      <div>
        ...
        {Array.from({length: stats.rolls}).map((_, i) => (
          <div key={i} className="flex gap-2 mb-2">
            <input value={skills[i] || ''} onChange={(e) => onSkillChange(i, e.target.value)} className="flex-1 bg-gray-700 px-3 py-2 rounded" placeholder={`Skill ${i+1}`} />
            <button onClick={() => onRandomSkill(i)} className="bg-blue-600 hover:bg-blue-500 px-4 py-2 rounded text-sm whitespace-nowrap" title="Randomly select from skills table">Random</button>
          </div>
        ))}
      </div>
      <button
