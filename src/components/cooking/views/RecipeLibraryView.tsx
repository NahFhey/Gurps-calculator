    ...
    <div className="bg-surface-1 rounded-lg p-6 space-y-4">
      {recipes.map(r => (
        <div key={r.id} className="bg-surface-2 rounded">
          <div className="flex items-center gap-3 p-4 cursor-pointer hover:bg-surface-3" onClick={() => onToggleExpanded(r.id)}>
            <span className="text-fg-muted">{expandedRecipes[r.id] ? <ChevronDown size={20} /> : <ChevronRight size={20} />}</span>
            <h3 className="flex-1 font-semibold text-lg">{r.name}</h3>
            <div className="flex gap-2">
              <button onClick={(e) => { e.stopPropagation(); onStartRemake(r); }} className="bg-success-600 px-4 py-2 rounded text-sm hover:bg-success-700">Make Recipe</button>
              <button onClick={(e) => { e.stopPropagation(); onDelete(r); }} className="bg-danger-600 px-4 py-2 rounded text-sm hover:bg-danger-700"><Trash2 size={16} /></button>
            </div>
          </div>
            ...
            <div className="px-4 pb-4 space-y-3 border-t border-edge-strong pt-3">
              <div className="text-sm space-y-1">
                ...
                {r.ingredients.map((ing, idx) => {
                  ...
                  return <div key={idx} className="ml-4">• {ing.amount} lbs {displayName} {displayTypes.length > 0 && <span className="text-accent-400">({displayTypes.join('/')})</span>}</div>;
                })}
              ...
              {r.creationHistory && r.creationHistory.length > 0 && (
                <div className="space-y-2">
                  ...
                  {r.creationHistory.map(log => (
                    <div key={log.id} className="bg-surface-3 p-3 rounded text-sm">
                      <div className="flex justify-between">
                        <span className={log.result === 'Critical Success' ? 'text-success-400' : log.result === 'Critical Failure' ? 'text-danger-400' : log.result === 'Success' ? 'text-accent-400' : 'text-fg-muted'}>{log.result}</span>
                        <span className="text-fg-muted">{new Date(log.date).toLocaleDateString()}</span>
                      </div>
