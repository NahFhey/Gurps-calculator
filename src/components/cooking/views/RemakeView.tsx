    ...
    <div className="bg-surface-1 rounded-lg p-6">
      ...
      <div className="space-y-4 mb-6">
        {ingredients.map((ing, ingIndex) => {
          ...
          return (
            <div key={ingIndex} className="bg-surface-2 p-4 rounded">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <div className="font-semibold">{ing.original.foodName}{foodTypes.length > 0 && <span className="text-accent-400 ml-2">({foodTypes.join('/')})</span>}</div>
                  <div className="text-sm text-fg-muted">Need: {ing.original.amount} lbs | Available: {originalFood ? originalFood.quantity : 0} lbs</div>
                </div>
              ...
              {!ing.useOriginal && (
                <div className="space-y-3 border-t border-edge-strong pt-3">
                  <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => onAddSubstitute(ingIndex)} className="bg-accent-600 px-3 py-1 rounded text-sm hover:bg-accent-700"><Plus size={14} className="inline" /> Add Substitute</button>
                  </div>
                  {ing.substitutes.map((sub, subIndex) => (
                    <div key={subIndex} className="bg-surface-3 p-3 rounded space-y-2">
                      <div className="flex gap-2">
                        <select value={sub.foodId || ''} onChange={(e) => onUpdateSubstitute(ingIndex, subIndex, 'foodId', e.target.value)} className="flex-1 bg-surface-2 px-3 py-2 rounded">
                          <option value="">Select substitute...</option>
                          {foods.filter(f => f.quantity > 0).map(f => <option key={f.id} value={f.id}>{f.name} ({f.types.join('/')}) - {f.quantity} lbs</option>)}
                        </select>
                        <input type="number" value={sub.amount} onChange={(e) => onUpdateSubstitute(ingIndex, subIndex, 'amount', parseFloat(e.target.value) || 1)} className="w-24 bg-surface-2 px-3 py-2 rounded" min="0.1" />
                        <button onClick={() => onRemoveSubstitute(ingIndex, subIndex)} className="text-danger-400 hover:text-danger-300"><Trash2 size={20} /></button>
                      </div>
                    </div>
                  ))}
                  <div className="bg-surface-3 p-3 rounded">
                    <label className="block text-sm mb-2">Substitution Penalty</label>
                    <select value={ing.penalty} onChange={(e) => onPenaltyChange(ingIndex, parseInt(e.target.value))} className="w-full bg-surface-2 px-3 py-2 rounded">
                    ...
                    <div className="text-xs text-fg-muted mt-1">Auto-calculated based on first substitute, adjust if needed</div>
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="bg-surface-2 p-4 rounded space-y-3 mb-6">
        <h4 className="font-semibold">Cooking Roll</h4>
        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className="block text-xs mb-1">Worker</label>
            <select value={worker} onChange={(e) => onWorkerChange(e.target.value)} className="w-full bg-surface-3 px-3 py-2 rounded">
          ...
          <div>
            ...
            <select value={kitchenId} onChange={(e) => onKitchenChange(e.target.value)} className="w-full bg-surface-3 px-3 py-2 rounded">
          ...
          <div>
            ...
            <input type="number" value={skill} onChange={(e) => onSkillChange(e.target.value)} className="w-full bg-surface-3 px-3 py-2 rounded" placeholder="e.g., 14" />
          </div>
      ...
      <button onClick={onExecute} className="w-full bg-success-600 py-3 rounded font-semibold hover:bg-success-700">Cook Recipe (Difficulty: {difficulty})</button>
    </div>
  );
}
