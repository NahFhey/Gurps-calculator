...
export function StorageQuotaBanner() {
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-[100] flex justify-center p-3 pointer-events-none">
      <div className="bg-danger-950/95 border border-danger-500/60 rounded-xl shadow-2xl p-4 max-w-lg w-full pointer-events-auto backdrop-blur-sm">
        {/* Header */}
        <div className="flex items-start gap-3">
          <AlertTriangle className="h-5 w-5 text-danger-400 flex-shrink-0 mt-0.5" />
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-semibold text-danger-100">Storage Full</h3>
          ...
          <button
            ...
            className="p-1 rounded hover:bg-danger-800/50 text-danger-400 hover:text-danger-200 transition-colors flex-shrink-0"
            aria-label="Dismiss"
          >
            <X className="h-4 w-4" />
          </button>
        ...
        {breakdown.length > 0 && (
          <div className="mt-3 flex items-center gap-2 text-[10px] text-danger-400">
            <HardDrive className="h-3 w-3" />
            {breakdown.slice(0, 3).map((entry) => (
              <span key={entry.key} className="bg-danger-900/50 px-1.5 py-0.5 rounded">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {checkpointCount > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-danger-800/60 hover:bg-danger-700/60 text-danger-100 border border-danger-600/40 transition-colors"
            >
