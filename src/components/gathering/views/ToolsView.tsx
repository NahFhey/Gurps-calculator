...
export function ToolsView({ tools, foodTypes, materialTypes, saveTools, onDelete }: ToolsViewProps) {
  ...
  return (
    <div>
      ...
      {showAdd && (
        <div className="bg-surface-2 p-4 rounded mb-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs text-fg-muted mb-1">Tool Type</label>
              <select
                value={newToolType}
          ...
          <div>
            <label className="block text-xs text-fg-muted mb-1">Allowed Modes</label>
          ...
          <div>
            <label className="block text-xs text-fg-muted mb-1">Allowed Methods (empty = all)</label>
            <div className="flex flex-wrap gap-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-fg-muted mb-1">Skill Bonus</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-xs text-fg-muted mb-1">Durability (optional)</label>
              <input
                type="number"
          ...
          {newToolModes.includes('Foraging') && (
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-xs text-fg-muted">Yield Bonuses (Foraging)</label>
                <button
                  type="button"
                  onClick={() => setNewToolYieldBonuses([...newToolYieldBonuses, { typeId: '', dice: '1' }])}
                  className="text-xs bg-accent-600 px-2 py-1 rounded"
                >
              ...
              <div className="space-y-2">
                {newToolYieldBonuses.map((yb, idx) => (
                  <div key={idx} className="flex gap-2 items-center">
                    <select
                      ...
                      className="flex-1 bg-surface-3 px-2 py-1 rounded text-sm"
                    >
                    ...
                    <input
                      ...
                      placeholder="Dice"
                      className="w-16 bg-surface-3 px-2 py-1 rounded text-sm text-center"
                    />
                ...
                {newToolYieldBonuses.length === 0 && (
                  <p className="text-xs text-fg-faint italic">No yield bonuses configured</p>
                )}
              </div>
            </div>
          )}
          ...
          <div className="flex gap-2 pt-2">
            <button onClick={addTool} className="flex-1 bg-success-600 px-4 py-2 rounded">
      ...
      <div className="space-y-2">
        {tools.length === 0 ? (
          <p className="text-fg-faint italic">No tools defined. Add fishing rods, nets, etc.</p>
        ) : (
          tools.map(t => {
            const yieldBonuses = t.bonuses?.filter(b => b.type === 'yield_bonus') || [];
            return (
              <div key={t.id} className="bg-surface-2 p-3 rounded">
                <div className="flex justify-between items-center">
                  <div className="flex-1">
                    <div>
                      <span className="font-medium">{t.name}</span>
                      <span className="text-sm text-fg-muted ml-2">({(GATHERING_TOOL_TYPES as Record<string, string>)[t.toolType] || t.toolType})</span>
                      {t.bonuses?.find(b => b.type === 'skill_bonus')?.value && (t.bonuses?.find(b => b.type === 'skill_bonus')?.value || 0) > 0 && (
                        <span className="text-success-400 ml-2">+{t.bonuses?.find(b => b.type === 'skill_bonus')?.value}</span>
                      )}
                    </div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => editTool(t)} className="text-accent-400">
                      <Edit2 size={16} />
                    </button>
                    <button onClick={() => onDelete('tool', t.id, t.name)} className="text-danger-400">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
