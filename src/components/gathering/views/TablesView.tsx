  ...
  return (
    <div>
      ...
      {showAdd && (
        <div className="bg-surface-2 p-4 rounded mb-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs text-fg-muted mb-1">Table Type</label>
              <select
                value={newTableType}
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              <label className="text-xs text-fg-muted">Roll Method</label>
              <button
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="text-xs text-fg-muted">Table Entries</label>
              <button onClick={addTableEntry} className="text-sm text-accent-400">+ Add Entry</button>
            </div>
            {newTableEntries.map((entry, idx) => (
              <div key={entry.id} className="flex gap-2 mb-2 items-center">
                <input
                  ...
                  className="w-16 bg-surface-3 px-2 py-1 rounded"
                  placeholder="Roll"
                />
                <select
                  ...
                  className="w-24 bg-surface-3 px-2 py-1 rounded"
                >
                ...
                {entry.resultType === 'species' && (
                  <select
                    ...
                    className="flex-1 bg-surface-3 px-2 py-1 rounded"
                  >
          ...
          <div className="flex gap-2 pt-2">
            <button onClick={addTable} className="flex-1 bg-success-600 px-4 py-2 rounded">
      ...
      <div className="space-y-2">
        {tables.length === 0 ? (
          <p className="text-fg-faint italic">No tables defined. Create catch and event tables.</p>
        ) : (
          tables.map(t => (
            <div key={t.id} className="bg-surface-2 rounded">
              <div
              ...
              >
                ...
                <span className="text-sm text-fg-muted">{t.tableType}</span>
                <span className="text-xs bg-surface-3 px-2 py-1 rounded">{t.entries?.length || 0} entries</span>
              </div>
              {expanded[`table-${t.id}`] && (
                <div className="px-3 pb-3 border-t border-edge-strong pt-2">
                  <div className="text-sm text-fg-muted mb-2">Roll: {t.rollMethod}</div>
                  {t.entries?.length > 0 && (
                      ...
                      {t.entries.map(entry => {
                        ...
                        return (
                          <div key={entry.id} className="text-sm flex gap-2">
                            <span className="text-fg-muted w-8">{entry.rollValue}:</span>
                            <span>
