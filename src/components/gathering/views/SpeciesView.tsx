              ...
              {expanded[s.id] && (
                <div className="px-3 pb-3 space-y-2 border-t border-gray-600 pt-2">
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div><span className="text-gray-400">Meat:</span> {s.yieldMeatFormula} &rarr; {s.foodType}</div>
                    {s.secondaryMaterialType && (
                      <div><span className="text-gray-400">Secondary:</span> {s.yieldSecondaryFormula} &rarr; {s.secondaryNameOverride || `${s.name} ${s.secondaryMaterialType}`}</div>
                    )}
