  ...
  return (
    <div>
      ...
      {showAdd && (
        <div className="bg-surface-2 p-4 rounded mb-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs text-fg-muted mb-1">Yield Formula</label>
              <input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-fg-muted mb-1">Inventory Type</label>
              <select
            ...
            <div>
              <label className="block text-xs text-fg-muted mb-1">Type ID</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-fg-muted mb-1">Rarity</label>
              <select
                value={newItemRarity}
          ...
          <div className="flex gap-2 pt-2">
            <button onClick={addItem} className="flex-1 bg-success-600 px-4 py-2 rounded">
      ...
      <div className="space-y-2">
        {items.length === 0 ? (
          <p className="text-fg-faint italic">No items defined. Add specific forageable items!</p>
        ) : (
          items.map(i => {
            const rarityData = FORAGING_RARITIES[i.rarity as keyof typeof FORAGING_RARITIES];
            return (
              <div key={i.id} className="bg-surface-2 rounded">
                <div
                ...
                >
                  ...
                  <span className="text-sm text-fg-muted">{i.yieldFormula}</span>
                ...
                {expanded[`item-${i.id}`] && (
                  <div className="px-3 pb-3 space-y-2 border-t border-edge-strong pt-2">
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div><span className="text-fg-muted">Type ID:</span> {i.typeId || 'N/A'}</div>
                      <div><span className="text-fg-muted">Yield:</span> {i.yieldFormula}</div>
                      <div><span className="text-fg-muted">Inventory:</span> {i.inventoryKind || 'food'}</div>
                      <div><span className="text-fg-muted">Rarity:</span> {rarityData?.label || i.rarity} ({rarityData?.penalty || 0})</div>
                    </div>
                    {i.description && (
                      <div className="text-sm text-fg-muted">{i.description}</div>
                    )}
