      ...
      {view === 'materials' && (
        <div className="bg-surface-1 rounded-lg p-6">
          <div className="flex justify-between mb-4">
            ...
            <button onClick={() => setShowAddMat(!showAddMat)} className="bg-success-600 px-4 py-2 rounded"><Plus size={20} className="inline" /> Add</button>
          </div>
          {showAddMat && (
            <div className="bg-surface-2 p-4 rounded mb-4 space-y-2">
              ...
              />
              <div className="flex gap-2">
                <button onClick={onAddMaterial} className="flex-1 bg-success-600 px-4 py-2 rounded">
          ...
          <div className="space-y-2">
            {materials.map(m => (
              <div key={m.id} className="bg-surface-2 rounded">
                <div className="flex items-center gap-4 p-3 cursor-pointer hover:bg-surface-3" onClick={() => setExpanded(p => ({...p, [`mat-${m.id}`]: !p[`mat-${m.id}`]}))}>
                  <span className="flex-1">{m.name}</span>
                  <span className="text-accent-400 text-sm">{m.type || 'no type'}</span>
                  <span className="text-right text-fg-muted">
                    {m.quantity} lbs
                    {ownerBreakdown(materialBreakdowns[m.id]) && (
                      <span className="block text-xs text-fg-faint">— {ownerBreakdown(materialBreakdowns[m.id])}</span>
                    )}
                  </span>
                  <span className="text-fg-muted">{expanded[`mat-${m.id}`] ? '▼' : '▶'}</span>
                </div>
                {expanded[`mat-${m.id}`] && (
                  <div className="px-3 pb-3 space-y-3 border-t border-edge-strong pt-3">
                    <div className="flex gap-4 items-end">
                      <div className="flex-1">
                        ...
                        <input value={m.name} onChange={(e) => onSaveMaterials(materials.map(x => x.id === m.id ? {...x, name: e.target.value} : x))} className="w-full bg-surface-3 px-3 py-1 rounded" />
                      </div>
                      <div className="w-32">
                        <label className="block text-xs text-fg-muted mb-1">Quantity</label>
                        <input type="number" value={m.quantity} onChange={(e) => onSaveMaterials(materials.map(x => x.id === m.id ? {...x, quantity: Math.max(0, toNumberOr(e.target.value, m.quantity))} : x))} className="w-full bg-surface-3 px-3 py-1 rounded" />
                      </div>
                      <span className="text-fg-muted pb-1">lbs</span>
                    </div>
                    <div>
                      <label className="block text-xs text-fg-muted mb-1">Type {!gmMode && <span className="text-yellow-400">(GM mode required to edit)</span>}</label>
                      <select
                        value={m.type || ''}
                    ...
                    <button onClick={() => setDeleteConfirm({type:'mat', id:m.id, name:m.name})} className="w-full bg-danger-600 py-2 rounded text-sm"><Trash2 size={16} className="inline" /> Delete</button>
                  </div>
                )}
              </div>
            ))}
      ...
      {view === 'foods' && (
        <div className="bg-surface-1 rounded-lg p-6">
          <div className="flex justify-between mb-4">
            ...
            <button onClick={() => setShowAddFood(!showAddFood)} className="bg-success-600 px-4 py-2 rounded"><Plus size={20} className="inline" /> Add</button>
          </div>
          {showAddFood && (
            <div className="bg-surface-2 p-4 rounded mb-4 space-y-3">
                ...
                <>
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      }} className="bg-accent-600 px-3 py-1 rounded text-sm"><Plus size={14} className="inline" /> Add</button>}
                    </div>
                    {newFoodTypes.map((t, i) => (
                      <div key={i} className="flex gap-2 mb-2">
                        <select value={t} onChange={(e) => { const u = [...newFoodTypes]; u[i] = e.target.value; setNewFoodTypes(u); }} className="flex-1 bg-surface-3 px-3 py-1 rounded">
                          {foodTypes.map(ft => {
                        ...
                        <button onClick={() => setNewFoodTypes(newFoodTypes.filter((_, j) => j !== i))} className="text-danger-400"><X size={18} /></button>
                      </div>
                    ))}
                  </div>
                </>
              ...
              />
              <div className="flex gap-2">
                <button onClick={onAddFood} className="flex-1 bg-success-600 py-2 rounded">
          ...
          <div className="space-y-4">
            {Object.keys(grouped).sort().map(k => {
              ...
              return (<div key={k}>
                <div className="bg-surface-2 px-4 py-2 rounded font-semibold text-sm" style={{color: typeColor}}>{k === 'no-type' ? 'No Type' : k}</div>
                {grouped[k].map(f => (
                  <div key={f.id} className="bg-surface-2 rounded mt-2">
                    <div className="flex items-center gap-4 p-3 cursor-pointer hover:bg-surface-3" onClick={() => setExpanded(p => ({...p, [f.id]: !p[f.id]}))}>
                      ...
                      <span className="text-right text-fg-muted">
                        ...
                        {ownerBreakdown(foodBreakdowns[f.id]) && (
                          <span className="block text-xs text-fg-faint">— {ownerBreakdown(foodBreakdowns[f.id])}</span>
                        )}
                      </span>
                      <span className="text-fg-muted">{expanded[f.id] ? '▼' : '▶'}</span>
                    </div>
                    {expanded[f.id] && (
                      <div className="px-3 pb-3 space-y-3 border-t border-edge-strong pt-3">
                        <div className="flex gap-4">
                          <input value={f.name} onChange={(e) => onSaveFoods(foods.map(x => x.id === f.id ? {...x, name: e.target.value} : x))} className="flex-1 bg-surface-3 px-3 py-1 rounded" />
                          <input type="number" value={f.quantity} onChange={(e) => onSaveFoods(foods.map(x => x.id === f.id ? {...x, quantity: Math.max(0, toNumberOr(e.target.value, f.quantity))} : x))} className="w-24 bg-surface-3 px-3 py-1 rounded" />
                          <span>lbs</span>
                          <button onClick={() => setDeleteConfirm({type:'food', id:f.id, name:f.name})} className="text-danger-400"><Trash2 size={20} /></button>
                        </div>
                        <div>
                          <div className="flex gap-2 mb-2">
                            ...
                            }} className="bg-accent-600 px-3 py-1 text-sm rounded"><Plus size={14} className="inline" /> Add</button>}
                          </div>
                          {(f.types || []).map((t, i) => {
                            ...
                            return (<div key={i} className="flex gap-2 mb-2 items-center">
                              ...
                              {gmMode && <button onClick={() => onSaveFoods(foods.map(x => x.id === f.id ? {...x, types: (x.types || []).filter((_, j) => j !== i)} : x))} className="text-danger-400"><X size={18} /></button>}
                            </div>);
                          })}
                        </div>
                      </div>
