        ...
        <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
          <main className="min-h-0 flex-1 overflow-y-auto p-5">
            <nav className="mb-5 flex gap-2 border-b border-edge pb-3">
              {([['skills', 'Skills & Spells'], ['attributes', 'Attributes'], ['traits', 'Traits']] as const).map(([key, label]) => <button key={key} type="button" onClick={() => setTab(key)} className={`rounded px-3 py-2 text-sm ${tab === key ? 'bg-accent-600 text-white' : 'bg-surface-2 text-fg-secondary hover:bg-surface-3'}`}>{label}</button>)}
            </nav>

            {tab === 'skills' && <div className="space-y-5">
              <section><h3 className="mb-2 font-medium text-fg-primary">Existing skills</h3><div className="space-y-2">{gcsData.skills.map((skill) => {
                ...
                return <div key={skill.id} className="flex items-center justify-between rounded bg-surface-0/70 p-3"><div><span className="text-fg-bright">{skill.name}</span><span className="ml-2 text-xs text-fg-muted">Level {skill.level} → {level}; +{points} point{points === 1 ? '' : 's'}</span></div><button data-testid={`add-skill-point-${skill.id}`} type="button" onClick={() => addIncrement({ id: `skill-${skill.id}`, kind: 'skill', skillId: skill.id, points: 1 })} className="rounded bg-emerald-700 p-1.5 text-white hover:bg-emerald-600"><Plus className="h-4 w-4" /></button></div>;
              })}</div></section>
              <section><h3 className="mb-2 font-medium text-fg-primary">Spells</h3><div className="space-y-2">{gcsData.spells.map((spell) => {
                ...
                return <div key={spell.id} className="flex items-center justify-between rounded bg-surface-0/70 p-3"><div><span className="text-fg-bright">{spell.name}</span><span className="ml-2 text-xs text-fg-muted">Level {spell.level} → {level}; +{points} points</span></div><button data-testid={`add-spell-point-${spell.id}`} type="button" onClick={() => addIncrement({ id: `spell-${spell.id}`, kind: 'spell', spellId: spell.id, points: 1 })} className="rounded bg-emerald-700 p-1.5 text-white"><Plus className="h-4 w-4" /></button></div>;
              })}</div></section>
              <section className="rounded border border-edge p-3"><h3 className="mb-3 font-medium text-fg-primary">Add new skill</h3><div className="grid gap-2 sm:grid-cols-4"><input aria-label="New skill name" value={newSkillName} onChange={(event) => setNewSkillName(event.target.value)} placeholder="Skill name" className="rounded border border-edge-strong bg-surface-0 px-3 py-2 text-fg-bright" /><input aria-label="New skill specialization" value={newSkillSpecialization} onChange={(event) => setNewSkillSpecialization(event.target.value)} placeholder="Specialization (optional)" className="rounded border border-edge-strong bg-surface-0 px-3 py-2 text-fg-bright" /><select aria-label="New skill attribute" value={newSkillAttribute} onChange={(event) => setNewSkillAttribute(event.target.value as SkillAttribute)} className="rounded border border-edge-strong bg-surface-0 px-2 text-fg-bright">{(['ST', 'DX', 'IQ', 'HT', 'Will', 'Per'] as const).map((value) => <option key={value}>{value}</option>)}</select><select aria-label="New skill difficulty" value={newSkillDifficulty} onChange={(event) => setNewSkillDifficulty(event.target.value as SkillDifficulty)} className="rounded border border-edge-strong bg-surface-0 px-2 text-fg-bright">{(['E', 'A', 'H', 'VH'] as const).map((value) => <option key={value}>{value}</option>)}</select></div><button type="button" onClick={addNewSkill} className="mt-2 rounded bg-accent-600 px-3 py-2 text-sm text-white">Add skill to cart</button></section>
            </div>}

            {tab === 'attributes' && <div className="space-y-5">
              <section><h3 className="mb-2 font-medium text-fg-primary">Primary attributes</h3><div className="grid gap-2 sm:grid-cols-2">{(Object.keys(ATTRIBUTE_COSTS) as Array<keyof PrimaryAttributes>).map((attribute) => {
                const increments = primaryIncrement(attribute); return <div key={attribute} className="flex items-center justify-between rounded bg-surface-0/70 p-3"><span>{attribute} {gcsData.attributes[attribute]} → {gcsData.attributes[attribute] + increments} <small className="text-fg-muted">({ATTRIBUTE_COSTS[attribute]} pts)</small></span><button data-testid={`add-attribute-${attribute}`} type="button" onClick={() => addIncrement({ id: `primary-${attribute}`, kind: 'primary', attribute, increments: 1 })} className="rounded bg-emerald-700 p-1.5"><Plus className="h-4 w-4" /></button></div>;
              })}</div></section>
              <section><h3 className="mb-2 font-medium text-fg-primary">Secondary characteristics</h3><div className="grid gap-2 sm:grid-cols-2">{(Object.keys(SECONDARY_COSTS) as Array<keyof SecondaryAttributes>).map((attribute) => {
                const increments = secondaryIncrementsInCart(lines, attribute); const step = attribute === 'basicSpeed' ? 0.25 : 1; return <div key={attribute} className="flex items-center justify-between rounded bg-surface-0/70 p-3"><span>{secondaryLabels[attribute]} {gcsData.secondaryAttributes[attribute].value} → {gcsData.secondaryAttributes[attribute].value + increments * step} <small className="text-fg-muted">({SECONDARY_COSTS[attribute]} pts)</small></span><button data-testid={`add-secondary-${attribute}`} type="button" onClick={() => addIncrement({ id: `secondary-${attribute}`, kind: 'secondary', attribute, increments: 1 })} className="rounded bg-emerald-700 p-1.5"><Plus className="h-4 w-4" /></button></div>;
              })}</div></section>
              <section><h3 className="mb-2 font-medium text-fg-primary">Point pools</h3><div className="grid gap-2 sm:grid-cols-2">{(['HP', 'FP'] as const).map((pool) => { const increments = poolIncrementsInCart(lines, pool); return <div key={pool} className="flex items-center justify-between rounded bg-surface-0/70 p-3"><span>{pool} max {gcsData.pools[pool].max} → {gcsData.pools[pool].max + increments} <small className="text-fg-muted">({POOL_COSTS[pool]} pts)</small></span><button data-testid={`add-pool-${pool}`} type="button" onClick={() => addIncrement({ id: `pool-${pool}`, kind: 'pool', pool, increments: 1 })} className="rounded bg-emerald-700 p-1.5"><Plus className="h-4 w-4" /></button></div>; })}</div></section>
              {(primaryIncrement('DX') > 0 || primaryIncrement('IQ') > 0) && <section data-testid="dependent-skill-preview" className="rounded border border-accent-800 bg-accent-950/30 p-3"><h3 className="mb-1 text-sm font-medium text-accent-200">Dependent skill preview</h3>{gcsData.skills.filter((skill) => skill.attribute === 'DX' || skill.attribute === 'IQ').map((skill) => { const increase = primaryIncrement(skill.attribute as 'DX' | 'IQ'); return <p key={skill.id} className="text-xs text-fg-secondary">{skill.name}: {skill.level} → {skill.level + increase}</p>; })}{primaryIncrement('IQ') > 0 && gcsData.spells.map((spell) => <p key={spell.id} className="text-xs text-fg-secondary">{spell.name}: {spell.level} → {spell.level + primaryIncrement('IQ')}</p>)}</section>}
            </div>}

            {tab === 'traits' && <section className="rounded border border-edge p-4"><h3 className="mb-3 font-medium text-fg-primary">Add GM-priced trait</h3><div className="grid gap-2 sm:grid-cols-3"><input aria-label="Trait name" value={traitName} onChange={(event) => setTraitName(event.target.value)} placeholder="Trait name" className="rounded border border-edge-strong bg-surface-0 px-3 py-2 text-fg-bright" /><select aria-label="Trait type" value={traitType} onChange={(event) => { const value = event.target.value as TraitSpendType; setTraitType(value); setTraitPoints(value === 'disadvantage' || value === 'quirk' ? -1 : 1); }} className="rounded border border-edge-strong bg-surface-0 px-3 py-2 text-fg-bright"><option value="advantage">Advantage</option><option value="perk">Perk</option><option value="disadvantage">Disadvantage</option><option value="quirk">Quirk</option></select><input aria-label="Trait point cost" type="number" value={traitPoints} onChange={(event) => setTraitPoints(Number(event.target.value) || 0)} className="rounded border border-edge-strong bg-surface-0 px-3 py-2 text-fg-bright" /></div><p className="mt-2 text-xs text-fg-muted">Negative costs credit the point pool.</p><button data-testid="add-trait-to-cart" type="button" onClick={addTrait} className="mt-3 rounded bg-accent-600 px-3 py-2 text-sm text-white">Add trait to cart</button></section>}
          </main>
