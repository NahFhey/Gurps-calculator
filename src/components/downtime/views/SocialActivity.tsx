...
function modifierClass(value: number): string {
  return value > 0 ? 'bg-emerald-900/60 text-emerald-300' : value < 0 ? 'bg-danger-900/60 text-danger-300' : 'bg-surface-2 text-fg-secondary';
}
...
function ContactCard({ contact, onSave, onDelete, locations }: ContactCardProps) {
  ...
  return (
    <article className="rounded-lg border border-edge bg-surface-1/60 p-3" data-testid="contact-card">
      {editing ? (
        <div data-testid="contact-edit-form">
          <div className="mb-2 grid grid-cols-2 gap-2"><input aria-label="Contact name" value={name} onChange={(event) => setName(event.target.value)} className="rounded border border-edge-strong bg-surface-0 px-2 py-1.5 text-sm text-fg-bright" /><select aria-label="Contact kind" value={kind} onChange={(event) => setKind(event.target.value as ContactKind)} className="rounded border border-edge-strong bg-surface-0 px-2 py-1.5 text-sm text-fg-bright"><option value="person">Person</option><option value="faction">Faction</option><option value="settlement">Settlement</option></select></div>
          <textarea aria-label="Contact notes" value={notes} onChange={(event) => setNotes(event.target.value)} className="mb-2 w-full rounded border border-edge-strong bg-surface-0 px-2 py-1.5 text-sm text-fg-bright" />
          <select aria-label="Contact location" value={locationId} onChange={(event) => setLocationId(event.target.value)} className="mb-2 w-full rounded border border-edge-strong bg-surface-0 px-2 py-1.5 text-sm text-fg-bright">
            <option value="">— nowhere in particular —</option>
            {locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}
          </select>
          <label className="mb-3 block text-sm text-fg-secondary">Standing <input aria-label="Contact modifier" type="number" min={-4} max={4} value={modifier} onChange={(event) => setModifier(Number(event.target.value))} className="ml-2 w-20 rounded border border-edge-strong bg-surface-0 px-2 py-1 text-fg-bright" /></label>
          <div className="flex gap-2"><button type="button" onClick={save} data-testid="save-contact-button" className="rounded bg-rose-600 px-3 py-1.5 text-sm text-white">Save</button><button type="button" onClick={() => setEditing(false)} className="rounded border border-edge-strong px-3 py-1.5 text-sm text-fg-secondary">Cancel</button></div>
        </div>
      ) : (
        <>
          <div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h5 className="font-medium text-fg-bright">{contact.name}</h5><span className="rounded bg-surface-2 px-1.5 py-0.5 text-xs capitalize text-fg-secondary">{contact.kind}</span><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${modifierClass(contact.modifier)}`} data-testid="contact-modifier-badge">{formatSigned(contact.modifier)}</span></div>{contact.notes && <p className="mt-1 line-clamp-2 text-sm text-fg-muted">{contact.notes}</p>}</div><div className="flex gap-2"><button type="button" onClick={() => setEditing(true)} aria-label={`Edit ${contact.name}`} className="text-fg-muted hover:text-fg-primary"><Pencil className="h-4 w-4" /></button><button type="button" onClick={() => onDelete(contact)} aria-label={`Delete ${contact.name}`} className="text-danger-400 hover:text-danger-300"><Trash2 className="h-4 w-4" /></button></div></div>
          <button type="button" onClick={() => setHistoryOpen((open) => !open)} className="mt-3 flex items-center gap-1 text-xs text-fg-muted" data-testid="contact-history-toggle">{historyOpen ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />} History ({contact.history.length})</button>
          {historyOpen && <ul className="mt-2 space-y-1 border-l border-edge pl-3 text-xs text-fg-muted" data-testid="contact-history">{contact.history.length === 0 ? <li>No shifts recorded</li> : [...contact.history].reverse().map((shift) => <li key={shift.id}>Day {shift.dayKey}: {formatSigned(shift.delta)} → {formatSigned(shift.newModifier)} — {shift.cause}</li>)}</ul>}
        </>
      )}
    </article>
  );
...
export function SocialActivity({ currentDayKey, currentSlot }: SocialActivityProps) {
  ...
  return (
    <div data-testid="social-activity">
      <header className="mb-4 flex items-center justify-between"><div className="flex items-center gap-2"><Users className="h-5 w-5 text-rose-400" /><h3 className="text-lg font-semibold text-fg-bright">Social</h3></div>{!isCreating && !resolvingTask && <button type="button" onClick={() => setIsCreating(true)} data-testid="new-social-task-button" className="flex items-center gap-1 rounded bg-rose-600 px-3 py-1.5 text-sm text-white hover:bg-rose-700"><Plus className="h-4 w-4" /> New Task</button>}</header>
      ...
      <section className="mb-6" data-testid="contact-ledger"><div className="mb-2 flex items-center justify-between"><h4 className="font-medium text-fg-primary">Ledger ({contacts.length})</h4>{!isAddingContact && <button type="button" onClick={() => setIsAddingContact(true)} data-testid="add-contact-button" className="flex items-center gap-1 rounded border border-rose-500/50 px-2 py-1 text-sm text-rose-300"><Plus className="h-3 w-3" /> Add contact</button>}</div>
        {isAddingContact && <div className="mb-3 rounded border border-edge bg-surface-1/60 p-3" data-testid="add-contact-form"><div className="grid grid-cols-2 gap-2"><input aria-label="New contact name" value={newName} onChange={(event) => setNewName(event.target.value)} placeholder="Contact name" className="rounded border border-edge-strong bg-surface-0 px-2 py-1.5 text-sm text-fg-bright" /><select aria-label="New contact kind" value={newKind} onChange={(event) => setNewKind(event.target.value as ContactKind)} className="rounded border border-edge-strong bg-surface-0 px-2 py-1.5 text-sm text-fg-bright"><option value="person">Person</option><option value="faction">Faction</option><option value="settlement">Settlement</option></select></div><textarea aria-label="New contact notes" value={newNotes} onChange={(event) => setNewNotes(event.target.value)} placeholder="Notes (optional)" className="my-2 w-full rounded border border-edge-strong bg-surface-0 px-2 py-1.5 text-sm text-fg-bright" /><div className="flex gap-2"><button type="button" onClick={handleAddContact} disabled={!newName.trim()} data-testid="save-new-contact-button" className="rounded bg-rose-600 px-3 py-1.5 text-sm text-white disabled:bg-surface-2">Add</button><button type="button" onClick={() => setIsAddingContact(false)} className="rounded border border-edge-strong px-3 py-1.5 text-sm text-fg-secondary">Cancel</button></div></div>}
        {contacts.length === 0 ? <p className="text-sm italic text-fg-muted">No contacts in the ledger</p> : <div className="space-y-2">{contacts.map((contact) => <ContactCard key={contact.id} contact={contact} locations={locations} onSave={handleSaveContact} onDelete={(entry) => { if (window.confirm(`Delete ${entry.name}?`)) campaignActions.removeContact(entry.id); }} />)}</div>}
      </section>
      ...
      {!resolvingTask && <><section className="mb-6" data-testid="pending-tasks-section"><h4 className="mb-2 font-medium text-fg-primary">Pending ({pendingTasks.length})</h4>{pendingTasks.length === 0 ? <p className="text-sm italic text-fg-muted">No pending social tasks</p> : <div className="space-y-2">{pendingTasks.map((task) => <SocialTaskCard key={task.id} task={task} leader={characters.find((character) => character.id === task.leaderId)} contact={selectContacts(campaignState)[task.activityData.contactId]} onResolve={() => setResolvingTask(task)} onCancel={() => cancel(task.id)} />)}</div>}</section><section data-testid="completed-tasks-section"><h4 className="mb-2 font-medium text-fg-primary">Completed ({completedTasks.length})</h4>{completedTasks.length === 0 ? <p className="text-sm italic text-fg-muted">No completed social tasks</p> : <div className="space-y-2">{completedTasks.map((task) => <SocialTaskCard key={task.id} task={task} leader={characters.find((character) => character.id === task.leaderId)} contact={selectContacts(campaignState)[task.activityData.contactId]} readonly />)}</div>}</section></>}
    </div>
  );
}
