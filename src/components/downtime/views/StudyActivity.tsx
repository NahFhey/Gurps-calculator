  ...
  return (
    <div data-testid="study-activity">
      <header className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2"><GraduationCap className="h-5 w-5 text-cyan-400" /><h3 className="text-lg font-semibold text-fg-bright">Study</h3></div>
        <div className="flex flex-wrap items-center gap-2">
          <label className="flex items-center gap-2 text-xs text-fg-secondary">
            Hours per point
            <input type="number" min={1} value={studyConfig.hoursPerPoint} onChange={(event) => campaignActions.setStudyConfig({ hoursPerPoint: Math.max(1, Number(event.target.value) || 1) })} data-testid="hours-per-point-input" className="w-20 rounded border border-edge-strong bg-surface-0 px-2 py-1 text-fg-bright" />
          </label>
      ...
      <section className="mb-6" data-testid="study-projects-section">
        <h4 className="mb-2 font-medium text-fg-primary">Projects ({projects.length})</h4>
        {projects.length === 0 ? <p className="text-sm italic text-fg-muted">No active study projects</p> : (
          <div className="space-y-2">{projects.map((project) => {
            ...
            return (
              <article key={project.id} className="rounded-lg border border-edge bg-surface-1/60 p-3" data-testid="study-project-card">
                <div className="mb-2 flex items-start justify-between gap-3">
                  <div><p className="font-medium text-fg-bright">{character?.name ?? project.characterId} — {displaySkillName(project.skillName, project.specialization)}</p><p className="text-xs text-fg-muted">Points awarded: {project.pointsAwarded}</p></div>
                  <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={() => { if (window.confirm(`Abandon study of ${project.skillName}?`)) campaignActions.removeStudyProject(project.id); }} aria-label={`Abandon ${project.skillName} study project`} className="text-danger-400 hover:text-danger-300"><Trash2 className="h-4 w-4" /></button>
                  </div>
                </div>
                <div className="h-2 overflow-hidden rounded bg-surface-0"><div className="h-full bg-cyan-500" style={{ width: `${progress}%` }} /></div>
                <p className="mt-1 text-right text-xs text-fg-muted">{project.accumulatedHours}/{studyConfig.hoursPerPoint}h</p>
              </article>
            );
          })}</div>
        )}
