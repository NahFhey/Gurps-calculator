...
export function StudyTaskCard({ task, leader, teacher, onComplete, onCancel, readonly = false }: StudyTaskCardProps) {
  ...
  return (
    <div className={`rounded-lg border-2 bg-gray-800/60 p-3 ${getStatusBorderColor(task.status)}`} data-testid="study-task-card" data-task-id={task.id}>
      <div className="mb-2 flex items-start justify-between">
        <div className="flex items-center gap-2"><GraduationCap className="h-4 w-4 text-cyan-400" /><span className="font-medium text-gray-100">{displaySkill}</span></div>
        <StatusBadge status={task.status} />
      </div>
      <div className="mb-3 space-y-1 text-sm text-gray-300">
        <p><span className="font-medium text-gray-200">Student:</span> {leader?.name ?? task.leaderId}</p>
        {teacher && <p><span className="font-medium text-gray-200">Teacher:</span> {teacher.name} <span className="rounded bg-cyan-900/50 px-1.5 py-0.5 text-xs text-cyan-300">{displaySkill}-{getTeacherLevel(teacher, task.activityData)}</span></p>}
        ...
        <p><span className="font-medium text-gray-200">This slot:</span> {hours}h</p>
      </div>
      ...
      {isPending && (
        <div className="mt-3 flex gap-2">
          ...
          {onCancel && <button type="button" onClick={onCancel} className="rounded border border-red-500/50 px-3 py-1.5 text-sm text-red-400 hover:bg-red-900/30">Cancel</button>}
        </div>
      )}
    </div>
  );
