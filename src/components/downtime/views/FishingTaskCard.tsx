...
import { Fish, Check, X, Clock, Loader, Ban } from 'lucide-react';
import type { DowntimeTask, FishingData, TaskStatus } from '../../../types/downtime';
...
export function FishingTaskCard({
  ...
  const speciesName = speciesData?.name ?? data.speciesId;
  ...
  return (
    <div
      className={`fishing-task-card p-3 rounded-lg border-2 bg-white ${statusBorders[task.status]}`}
    ...
    >
      ...
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">Fishing: {speciesName}</span>
        </div>
      ...
      <div className="task-details text-sm text-gray-600 space-y-1 mb-3">
        ...
        <p>
          <span className="font-medium">Spot:</span> {spotName}
        </p>
        <p>
        ...
        <p>
          <span className="font-medium">Target Yield:</span> {data.targetYield}
        </p>
      </div>
