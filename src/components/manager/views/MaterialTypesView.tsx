  ...
  return (
    <div>
      ...
      {showAdd && (
        <div className="bg-surface-2 p-4 rounded mb-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-fg-muted mb-1">Difficulty Modifier</label>
              <input
                type="number"
                value={newMatDiff}
            ...
            <div>
              <label className="block text-xs text-fg-muted mb-1">HT</label>
              <input
                type="number"
                value={newMatHT}
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs text-fg-muted mb-1">DR Shift</label>
              <input
                type="number"
                value={newMatDR}
            ...
            <div>
              <label className="block text-xs text-fg-muted mb-1">Weight Mod (%)</label>
              <input
                type="number"
                min="-100"
                max="100"
            ...
            <div>
              <label className="block text-xs text-fg-muted mb-1">HP Mod (%)</label>
              <input
                type="number"
                min="-100"
                max="100"
          ...
          <div>
            <label className="block text-xs text-fg-muted mb-1">Effects</label>
            <textarea
              value={newMatEffects}
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 bg-success-600 py-2 rounded"
            >
      ...
      <div className="space-y-2">
        {materialTypes.map(t => (
          <div key={t.name} className="bg-surface-2 rounded">
            <div
            ...
            >
              ...
              <span className="text-fg-muted text-sm">Diff: {t.difficulty >= 0 ? '+' : ''}{t.difficulty}</span>
              <span className="text-fg-muted text-sm">HT: {t.ht}</span>
              <span className="text-fg-muted text-sm">DR: {t.drShift >= 0 ? '+' : ''}{t.drShift}</span>
              <span className="text-accent-400 text-sm">W: {t.weightMod >= 0 ? '+' : ''}{t.weightMod}%</span>
              <span className="text-accent-400 text-sm">HP: {t.hpMod >= 0 ? '+' : ''}{t.hpMod}%</span>
              <span className="text-fg-muted">{expanded[t.name] ? '▼' : '▶'}</span>
            </div>

            {expanded[t.name] && (
              <div className="px-3 pb-3 space-y-3 border-t border-edge-strong pt-3">
                <div>
                  <label className="block text-xs text-fg-muted mb-1">Name (blur to save)</label>
                  <input
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block text-xs text-fg-muted mb-1">Difficulty</label>
                    <input
                      type="number"
                      value={t.difficulty}
                      ...
                      className="w-full bg-surface-3 px-2 py-1 rounded"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-fg-muted mb-1">HT</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs text-fg-muted mb-1">Weight Modifier (%)</label>
                    <input
                      type="number"
                      min="-100"
                      max="100"
                  ...
                  <div>
                    <label className="block text-xs text-fg-muted mb-1">HP Modifier (%)</label>
                    <input
                      type="number"
                      min="-100"
                      max="100"
