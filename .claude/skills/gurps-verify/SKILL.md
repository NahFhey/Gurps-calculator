...
```bash
cd ~/Meta/projects/gurps-charsheet && node --import tsx/esm packages/cli/src/index.ts vault-health "../../vault/Dnd"
```
**Check:** Compare structural ("parsed") and computational ("ready") rates against the recorded baseline in `PARSE_BASELINE.md` at the charsheet repo root. If either metric dropped, that's a regression — your parser change broke something it previously handled. If a drop is an intentional trade-off the user approved, update `PARSE_BASELINE.md` with the new rates and the reason; otherwise report REGRESSED. If the baseline file is missing, record the current census output there as the new baseline and report SKIP (nothing to compare against).
